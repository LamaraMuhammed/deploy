...
const Markers = {
  ...
  continueAutoMove: function(delay) {
    this.moveId.push(
      doLater(() => this.autoMove = true, delay || 11000)
    );
  },

  setState: function(x, y) {if (dom.registeredUser()) localStorage.setItem(x, y||"enabled")},

  isDefault: function (x) { return localStorage.getItem(x) === null ? true : false; },
  ...
  showDragHandle: async function(e) {
    if (dom.isMobile) {
      ...
      this.touchMarker = await new L.marker(
        e?e.latlng:map.getCenter(), { draggable: true, icon: this.touchIcon }
  ...
  doIllustration: function(e, r) {
    ...
    if (!dom.isMobile) {
      ...
      this.outerPointer = L.circle([this.x, this.y], { color: "#00ff00", weight: 1, fillColor:"", fillOpacity: 0, radius: 5, pane: 'shadowPane' }).addTo(map);
    }
    ...
    if (this.midPointer || this.X0 || this.Y0) {
     ...
     // vertical line
    this.X0 = L.polyline([[this.x, this.y],[this.x, 1.0]], {color: '#00ff00',weight: 1}).addTo(map);
    this.X1 = L.polyline([[this.x, this.y],[this.x, 100]], {color: '#00ff00',weight: 1}).addTo(map);
    
    // horizontal line
    this.Y0 = L.polyline([[this.x, this.y],[1.0, this.y]], {color: '#00ff00',weight: 1}).addTo(map);
    this.Y1 = L.polyline([[this.x, this.y],[100, this.y]], {color: '#00ff00',weight: 1}).addTo(map);
  
    this.midPointer = L.circle([this.x, this.y], { color: "#000000", weight: 0, fillColor:"black", fillOpacity: 0.7, radius: 2, }).addTo(map);
    this.focusOnMe(e, r);
  },
  ...
  deleteOneMarker: function(store, selfDel) {
    if (store.length > 0) {
      ...
      if (dom.defaultFrndToInteract && selfDel) Markers.delMarkerIndex = store[(store.length - 1)].options.index.toString();
  ...
  // Trip Reader
  readData: function() {
    let len = this.tripData.length, dt;
    if (!dom.defaultFrndToInteract && len > 0 && this.copyTripData.length === 0) {
  ...
  placeTheMarker: function() {
    let firstCoords = L.latLng(this.copyTripData[0].geometry.coordinates).equals(this.bbox[0]);
    let lastCoords = L.latLng(this.copyTripData[0].geometry.coordinates).equals(this.bbox[(this.bbox.length - 1)]);
    try {
      L.geoJSON(this.copyTripData[0], {
        onEachFeature: Markers.showPopUp,
        pointToLayer: function (feature) {
          Markers.absoluteFit(feature.geometry.coordinates, feature.geometry.coordinates, {
            animate: true, duration: 4,
            easeLinearity: 0.7,
            maxZoom: map.getZoom()
          });
          this.tripMarker = 
          firstCoords ? L.marker(feature.geometry.coordinates, { icon: Markers.remotePsBorderIcon }) :
          lastCoords ? L.marker(feature.geometry.coordinates, { icon: Markers.remotePsBorderIcon }) : 
          L.marker(feature.geometry.coordinates, { icon: Markers.remotePsMidIcon });
          Markers.tripMkList.push(this.tripMarker);
          this.tripMarker.on('dblclick', e => map.removeLayer(this.tripMarker));
    ...
    if (this.copyTripData.length === 0) {
      ...
      if (this.layered)  doLater(e => this.fitAfter(this.bbox[0], this.bbox[(this.bbox.length - 1)]), 2000);
  ...
  absoluteFit: function(x, y, option) {map.fitBounds([L.latLng(x), L.latLng(y)], option);},
...
function imageView(img) {
  if (img) {
    ...
    span.onclick = function () {dom.select(".customImgView").remove(); dom.fullViewImg = null;};
...
function ONE() {
  ...
  setInterval(() => {
    ...
    Markers.g01Marker = L.marker(
      ...
      Markers.activeUserIcon : Markers.guestIcon
    );
...
function THREE() {
  ...
  setInterval(() => {
    let data = {lat: `10.3018${(Math.random()*(11-10)+10).toString().slice(12)}`, lng: 9.8333898}
